...
namespace Magexperts\AutoRelatedProduct\Model\ResourceModel\Rule;
...
class Collection extends AbstractCollection implements RelatedCollectionInterface
{
     ...
     * @var string
     */
    protected $_eventPrefix = 'magexperts_arp_rule_collection';
    ...
    {
        $this->_init(\Magexperts\AutoRelatedProduct\Model\Rule::class, \Magexperts\AutoRelatedProduct\Api\RelatedResourceModelInterface::class);
    ...
    protected function _renderFiltersBefore()
    {
        foreach (['store', 'group'] as $key) {
            if ($this->getFilter($key)) {
                ...
                $this->getSelect()->join(
                    [$key . '_table' => $this->getTable('magexperts_autorp_rule_' . $key)],
    ...
    protected function _afterLoad()
    {
      /*  $items = $this->getColumnValues('id');
        if (count($items)) {
            ...
            $tableName = $this->getTable('magexperts_autorp_rule_store');
