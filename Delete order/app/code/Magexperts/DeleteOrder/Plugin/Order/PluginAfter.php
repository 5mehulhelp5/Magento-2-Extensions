 ...
 */
namespace Magexperts\DeleteOrder\Plugin\Order;
    ...
    public function afterGetBackUrl(\Magento\Sales\Block\Adminhtml\Order\View $subject, $result)
    {
        if ($this->isAllowedResources()) {
            ...
            if ($subject->getRequest()->getFullActionName() == 'sales_order_view') {
                $subject->addButton(
                    'magexperts-delete',
                    ['label' => __('Delete'), 'onclick' => 'confirmSetLocation(\'' . $message . '\',\'' . $this->getDeleteUrl($params['order_id']) . '\')', 'class' => 'magexperts-delete'],
                    -1
                );
            }
        }
