 ...
 */
class Save extends \Magexperts\Blog\Controller\Adminhtml\Post
{
    /**
     * @var string
     */
    protected $_allowedKey = 'Magexperts_Blog::post_save';

    /**
     * Before model save
     * @param  \Magexperts\Blog\Model\Post $model
    ...
    protected function _beforeSave($model, $request)
    {
                ...
                if (!empty($data[$key]['delete'])) {
                    ...
                    if (isset($data[$key][0]['name']) && isset($data[$key][0]['tmp_name'])) {
                        ...
                        $imageUploader = $this->_objectManager->get(
                            \Magexperts\Blog\ImageUpload::class
                        );
                        ...
                        if (isset($data[$key][0]['url']) && false !== strpos($data[$key][0]['url'], '/media/')) {
                             ...
                             *    /media/.renditions/magexperts_blog/a.png
                             *    http://crowdin.dev.magexperts.top/media/magexperts_blog/tmp/a.png
                             */
        ...
        if ($tagInput) {
            ...
            $tagsCollection = $this->_objectManager->create(\Magexperts\Blog\Model\ResourceModel\Tag\Collection::class);
            $allTags = [];
            ...
            foreach ($tagInput as $tagTitle) {
                ...
                if (empty($allTags[$tagTitle])) {
                    $tagModel = $this->_objectManager->create(\Magexperts\Blog\Model\Tag::class);
