...
use Magexperts\Blog\Model\Config\Source\PostsSortBy;
use Magexperts\Blog\Block\Post\PostList\Toolbar;

/**
 * Blog index block
 */
class Index extends \Magexperts\Blog\Block\Post\PostList
{
    /**
    ...
    protected function _prepareLayout()
    {
        ...
        if ($this->config->getDisplayCanonicalTag(\Magexperts\Blog\Model\Config::CANONICAL_PAGE_TYPE_INDEX)) {
    ...
    protected function _preparePostCollection()
    {
        ...
        $displayMode = $this->_scopeConfig->getValue(
            \Magexperts\Blog\Model\Config::XML_PATH_HOMEPAGE_DISPLAY_MODE,
            ScopeInterface::SCOPE_STORE
        );
        /* If featured posts enabled */
        if ($displayMode == 1) {
            $postIds = $this->_scopeConfig->getValue(
                \Magexperts\Blog\Model\Config::XML_PATH_HOMEPAGE_FEATURED_POST_IDS,
                ScopeInterface::SCOPE_STORE
            );
    ...
    public function getCollectionOrderField()
    {
        $postsSortBy = $this->_scopeConfig->getValue(
            \Magexperts\Blog\Model\Config::XML_PATH_HOMEPAGE_POSTS_SORT_BY,
            ScopeInterface::SCOPE_STORE
        );
