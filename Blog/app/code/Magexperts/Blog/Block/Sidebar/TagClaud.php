...
class TagClaud extends \Magento\Framework\View\Element\Template
{
     ...
     * @param \Magexperts\Blog\Model\ResourceModel\Tag\CollectionFactory $_tagCollectionFactory
     * @param array $data
     */
        ...
        if ($this->_tags === null) {
            ...
            $resource = $this->_tags->getResource();
                $this->_tags->getSelect()->joinLeft(
                    ['pt' => $resource->getTable('magexperts_blog_post_tag')],
                    'main_table.tag_id = pt.tag_id',
                    []
                )->joinLeft(
                    ['p' => $resource->getTable('magexperts_blog_post')],
                    'p.post_id = pt.post_id',
                    []
                )->joinLeft(
                    ['ps' => $resource->getTable('magexperts_blog_post_store')],
                    ...
                    \Magexperts\Blog\Model\Tag::STATUS_ENABLED
                )->order('count DESC');
    ...
    protected function _prepareLayout()
    {
        ...
        if ($this->getConfigValue(self::ANIMATED_ENABLED)) {
            $this->setTemplate('Magexperts_Blog::sidebar/tag_claud_animated.phtml');
        }
