...
namespace Magexperts\Blog\Model\ResourceModel\Comment;
...
class Collection extends AbstractCollection
{
    ...
    {
        $this->_init(\Magexperts\Blog\Model\Comment::class, \Magexperts\Blog\Model\ResourceModel\Comment::class);
    ...
    {
        return $this
            ->addFieldToFilter('status', \Magexperts\Blog\Model\Config\Source\CommentStatus::APPROVED);
    }
     ...
     * @param array|int|string|\Magexperts\Blog\Model\Post  $post
     * @return $this
     */
    public function addPostFilter($post)
    {
        if (!$this->getFlag('post_filter_added')) {
            if ($post instanceof \Magexperts\Blog\Model\Post) {
                $post = [$post->getId()];
            }
            ...
            $tableName = $this->getTable('magexperts_blog_post');
    ...
    protected function _renderFiltersBefore()
    {
        if ($this->getFilter('store') && !$this->getFlag('store_filtered')) {
            $this->getSelect()->join(
                ['store_table' => $this->getTable('magexperts_blog_post_store')],
