...
namespace Magexperts\Blog\Model\ResourceModel\Post;
...
class Collection extends \Magento\Framework\Model\ResourceModel\Db\Collection\AbstractCollection
{
    ...
    /**
     * @var \Magexperts\Blog\Model\Category
     */
    protected $category;

    /**
     * @var \Magexperts\Blog\Api\CategoryRepositoryInterface|null
     */
     ...
     * @param \Magexperts\Blog\Api\CategoryRepositoryInterface|null $categoryRepository
     */
    public function __construct(
        ...
        \Magexperts\Blog\Api\CategoryRepositoryInterface $categoryRepository = null
    ) {
        ...
        $this->categoryRepository = $categoryRepository ?: $objectManager->create(
            \Magexperts\Blog\Api\CategoryRepositoryInterface::class
        );
    }
    ...
    {
        parent::_construct();
        $this->_init(\Magexperts\Blog\Model\Post::class, \Magexperts\Blog\Model\ResourceModel\Post::class);
     ...
     * @param array|int|\Magexperts\Blog\Model\Category  $category
     * @return $this
     */
    public function addCategoryFilter($category)
    {
        if (!$this->getFlag('category_filter_added')) {
            if ($category instanceof \Magexperts\Blog\Model\Category) {
            ...
            $tableName = $this->getTable('magexperts_blog_category');
    ...
    public function addSearchFilter($term)
    {
        ...
        $select = $connection->select()
            ->from(
                ['p' => $this->getTable('magexperts_blog_post')],
                ['post_id']
            )->joinInner(
                ['pt' => $this->getTable('magexperts_blog_post_tag')],
                'p.post_id = pt.post_id',
                ['']
            )->joinInner(
                ['t' => $this->getTable('magexperts_blog_tag')],
                't.tag_id = pt.tag_id',
                ['tag_title' => 'title']
     ...
     * @param array|int|string|\Magexperts\Blog\Model\Tag  $tag
     * @return $this
     */
    public function addTagFilter($tag)
    {
        if (!$this->getFlag('tag_filter_added')) {
            if ($tag instanceof \Magexperts\Blog\Model\Tag) {
                $tag = [$tag->getId()];
            }
            ...
            $connection = $this->getConnection();
            $tableName = $this->getTable('magexperts_blog_tag');
     ...
     * @param array|int|\Magexperts\Blog\Model\Author  $author
     * @return $this
     */
    public function addAuthorFilter($author)
    {
        if (!$this->getFlag('author_filter_added')) {
            if ($author instanceof \Magexperts\Blog\Model\Author) {
                $author = [$author->getId()];
            }
            ...
            $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
            $authorModel = $objectManager->get(\Magexperts\Blog\Api\AuthorInterface::class);
    ...
    protected function _afterLoad()
    {
        ...
        if (count($items)) {
            $connection = $this->getConnection();
            $tableName = $this->getTable('magexperts_blog_post_store');
            $select = $connection->select()
                ->from(['cps' => $tableName])
            ...
            foreach ($map as $key => $property) {
                $tableName = $this->getTable('magexperts_blog_post_' . $key);
                $select = $connection->select()
                    ->from(['cps' => $tableName])
                    ->where('cps.post_id IN (?)', $items);
    ...
    protected function _renderFiltersBefore()
    {
        foreach (['store', 'category', 'tag', 'author', 'relatedproduct'] as $key) {
            if ($this->getFilter($key)) {
                ...
                $this->getSelect()->join(
                    [$key . '_table' => $this->getTable('magexperts_blog_post_' . $key)],
