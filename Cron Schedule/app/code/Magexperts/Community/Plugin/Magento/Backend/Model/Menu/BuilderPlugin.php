...
namespace Magexperts\Community\Plugin\Magento\Backend\Model\Menu;

use Magento\Backend\Model\Menu\Builder;
use Magento\Backend\Model\Menu;
use Magento\Backend\Model\Menu\ItemFactory;
use Magexperts\Community\Model\Config;
...
class BuilderPlugin
{
    ...
    public function __construct(
        ...
        $this->magexpertsModules = $this->getMagexpertsModules();
    }
    ...
    public function afterGetResult(Builder $subject, Menu $menu, $result)
    {
        ...
        if ($menuEnabled) {
            $item = $this->menuItemFactory->create([
                'data' => [
                    'id' => 'Magexperts_Community::elements',
                    'title' => 'Magexperts',
                    'module' => 'Magexperts_Community',
                    'resource' => 'Magexperts_Community::elements'
                ]
            ]);
            $menu->add($item, null, 61);
            $subItems = $this->getSubItem($menu->toArray());
            $this->createMenuItem($menu, $subItems, 'Magexperts_Community::elements');

            $item = $this->menuItemFactory->create([
                'data' => [
                    'id' => 'Magexperts_Community::extension_and_notification',
                    'title' => 'Extensions &amp; Notifications',
                    'module' => 'Magexperts_Community',
                    'resource' => 'Magexperts_Community::elements'
                ]
            ]);
            $menu->add($item, 'Magexperts_Community::elements', 1000);

            $item = $this->menuItemFactory->create([
                'data' => [
                    'id' => 'Magexperts_Community::extension_and_notification_view',
                    'title' => 'Manage',
                    'module' => 'Magexperts_Community',
                    'resource' => 'Magexperts_Community::elements',
            ...
            $menu->add($item, 'Magexperts_Community::extension_and_notification', 1000);

            unset($this->configSections['Magexperts_Community']);

            foreach ($this->magexpertsModules as $moduleName) {
                ...
                if (isset($section['id']) && 'mfextension' != $section['id']) {
                    ...
                    $menu->add($item, 'Magexperts_Community::elements');
    ...
    private function getConfigSections($moduleName)
    {
        if (null === $this->configSections) {
            ...
            foreach ($tabs as $tab) {
                if (in_array($tab->getId(), ['magexperts', 'mf_extensions_list'])) {
                    $sections = array_merge($sections, $tab->getData()['children']);
                }
            }

            foreach ($sections as $key => $section) {
                if (empty($section['resource']) || 0 !== strpos($section['resource'], 'Magexperts_')) {
                    continue;
                }
    ...
    private function createMenuItem($menu, $items, $parentId)
    {
        foreach ($items as $item) {
            ...
            $title = trim(str_replace('Magexperts_', '', $title));
            $needCreateMenuItem = ('Magexperts_Community::elements' == $parentId && !empty($item['action']));
            ...
            if ('Magexperts_Community::elements' == $parentId) {
                $addConfig = true;
                if (!empty($item['sub_menu'])) {
            ...
            $index = array_search($moduleName, $this->magexpertsModules);
            if (false !== $index) {
                unset($this->magexpertsModules[$index]);
            }
        }
    }
    ...
    private function getSubItem($items)
    {
        ...
        if (!empty($items)) {
            foreach ($items as $item) {
                if (isset($item['module']) && 0 === strpos($item['module'], 'Magexperts_')
                    || !isset($item['module']) && isset($item['id']) && 0 === strpos($item['id'], 'Magexperts_')
                ) {
                    if ('Magexperts_Community::elements' != $item['id']) {
                        $subItems[] = $item;
                    }
    ...
    /**
     * Retrieve Magexperts modules info
     *
     * @return array
     */
    private function getMagexpertsModules()
    {
        $modules = [];
        foreach ($this->moduleList->getNames() as $moduleName) {
            if (strpos($moduleName, 'Magexperts_') !== false && $this->moduleManager->isEnabled($moduleName)) {
                $modules[] = $moduleName;
            }
        }
        return $modules;
