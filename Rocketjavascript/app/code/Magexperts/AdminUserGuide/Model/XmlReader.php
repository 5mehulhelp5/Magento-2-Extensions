...
class XmlReader
{
    ...
    const DATA_FILE_NAME = 'magexperts/aug-data/knowledge-base.xml';
    ...
    public function get()
    {
        if (!$this->cache->load('magexperts_aug_data')) {
            $this->readFile();
        }

        try {
            return $this->json->unserialize($this->cache->load('magexperts_aug_data'));
        } catch (\Exception $e) {
            return [];
        }
    }
    ...
    private function readFile()
    {
        ...
        $this->cache->save($this->json->serialize($data),'magexperts_aug_data');
    }
